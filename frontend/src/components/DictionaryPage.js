...
const DictionaryPage = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(manyEntries);
    setIndividual(manyEntries);
  };

  function handleCcharChange(e) {
    let val = e.target.value;
    setCchars(val);
  }
  ...
  function createDefinitionView(entry, index) {
    return (
      <DefinitionView key={index}>
        <DefinitionHeader>
          <CCharView>{entry.simplified}</CCharView>
    ...
    <Container>
      ...
      <Display>
        ...
        {individual.length === 0 ? (
          ...
          <Collapsible trigger="Individual Chars" transitionTime={200} open>
            <IndividualView>
              {individual.map((ccharEntries, idx) => (
                <IndividualCharView key={idx}>
                  {ccharEntries.map((charEntry, idx2) =>
                    createDefinitionView(charEntry, idx2)
                  )}
                </IndividualCharView>
              ))}
            </IndividualView>
